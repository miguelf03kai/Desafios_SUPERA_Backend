...
public interface TransferenciaRepository extends PagingAndSortingRepository<Transferencia,Long>{
	...
	@Query("SELECT t FROM transferencia t WHERE (:startDate IS NULL OR t.dataTransferencia >= :startDate)"+
		   " AND (:endDate IS NULL OR t.dataTransferencia <= :endDate) AND (COALESCE(:operador, '') = ''"+
			" OR LOWER(t.nomeOperadorTransacao) LIKE LOWER(CONCAT('%', :operador , '%')))")
